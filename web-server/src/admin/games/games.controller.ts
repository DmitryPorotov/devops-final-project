...
export class GamesController {
    ...
    @Post(':id')
    async save(@Param('id') id: string, @Body() {name}: {name: string}, @Req() request: Request) {
    ...
    @Post(':lobbyId/load')
    async loadGame(@Param('lobbyId') lobbyId: string, @Body() {name}: {name: string}, @Req() request: Request) {
