...
class RedisRouterService {
    ...
    private workerSubscriptions: Map<number, Observable<WorkerMessageInterface>> = new Map<number, Observable<WorkerMessageInterface>>();
    private chatSubscriptions: Map<number, Observable<MessageInterface>> = new Map<number, Observable<MessageInterface>>();
    private isInit = false;

    constructor(private redisSubscribeService: RedisSubscribeService,
                private redisChatService: RedisChatService) {
    }

    async getPlayerMessagesObservable(playerId: number, lobbyId: number): Promise<Observable<MessageInterface | WorkerMessageInterface>> {
        if (!this.isInit) {
        ...
        const workerObs = this.workerSubscriptions.has(lobbyId)
            ...
            : this.workerSubscriptions.set(lobbyId, await this.redisSubscribeService.getLobbyMessagesFromWorkerObservable(lobbyId)).get(lobbyId);
        const chatObs = this.chatSubscriptions.has(lobbyId)
            ? this.chatSubscriptions.get(lobbyId)
            : this.chatSubscriptions.set(lobbyId, await this.redisChatService.getLobbyChatMessagesObservable(lobbyId)).get(lobbyId);
