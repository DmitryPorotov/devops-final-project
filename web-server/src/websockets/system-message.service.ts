...
class SystemMessageService {
    ...
    private readonly chats: Map<number, (msg: string) => void> = new Map<number, (msg:string) => void>();
    ...
    async relayToLobbies(client: WebsocketWithUserInterface, message: MessageInterface) {
    ...
    private callback(data: MessageInterface) {
        ...
        && this.lobbiesClientsMapService.get(data.lobbyId)
    ...
    async subscribeToSystemEvents(lobbyId: number) {
        ...
        await this.redisStreamListener.addListener(`${constants.CHAT_PREFIX}${lobbyId}sys`, id, chatCb);
    }

    async unsubscribeFromSystemEvents(lobbyId: number) {
        if (!this.chats.has(lobbyId)) return ;
        await this.redisStreamListener.removeListener(`${constants.CHAT_PREFIX}${lobbyId}sys`, this.chats.get(lobbyId))
    }
}
