...
class RedisStreamListener {
  ...
  async addListener(
    ...
    if (!this.listeners.hasOwnProperty(streamName) || once) {
      if (!once) {
        this.logger.debug(
          'adding new listener ' + streamName + ' id ' + startId,
        );
      ...
      const xread = async ({ stream, id }: { stream: string; id: string }) => {
          ...
          response.forEach((r) => {
            r.messages.forEach((message) => {
              ...
              if (once) {
                ...
                this.listeners[streamName].callbacks.forEach((c) =>
                  c(message.message.json),
                );
              }
            });
          });
        ...
        if (!once) {
          //recursive timeout
          this.listeners[streamName].timeout = setTimeout(
            async () =>
              await xread({
                stream,
                id,
  ...
  async send(streamName: string, message: string): Promise<string> {
    return await this.redisClient.xAdd(
      streamName,
      '*',
      { json: message },
      {
