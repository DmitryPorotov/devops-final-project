...
export default class ChatService implements MessagingProviderInterface {
    ...
    constructor(private readonly redisStreamListener: RedisStreamListener) {
    }

    async init(chatCallback: (message: Object) => void): Promise<void> {
        if (!this.isInit) {
    ...
    async sendToChat(lobbyId: number, message: MessageInterface): Promise<void> {
        ...
        await this.redisStreamListener.send(`${constants.CHAT_PREFIX}${lobbyId}`, JSON.stringify(message));
    }
    ...
    async getAllChatMessages(lobbyId: number, cb: (msg: Object) => void) {
        ...
        return await this.redisStreamListener.addListener(`${constants.CHAT_PREFIX}${lobbyId}`,'0', chatCb, true);
    }
    ...
    async subscribeToChat(lobbyId: number): Promise<void> {
        ...
        await this.redisStreamListener.addListener(`${constants.CHAT_PREFIX}${lobbyId}`, id, chatCb);
        this.logger.debug("added listener");
    }

    async unsubscribeFromChat(lobbyId: number): Promise<void> {
        if (!this.chats.has(lobbyId)) return ;
        this.redisStreamListener.removeListener(`${constants.CHAT_PREFIX}${lobbyId}`, this.chats.get(lobbyId));
