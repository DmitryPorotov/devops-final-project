...
class MessageNewGameSpec extends AnyFlatSpec with should.Matchers {

  private val TO_WORKERS_EXCHANGE = "to_workers"
  private val FROM_WORKERS_EXCHANGE = "from_workers"

  "The server" should "return the number of games it runs" in {
    ...
    val message = "{\"userId\":1,\"action\":\"new_game\"}"
    ...
    channel.basicConsume(queueName, true, (consumerTag, delivery: Delivery) => {
      ...
      val gamesCount = Try[Int](json.obj("gamesCount").num.toInt) getOrElse -1
      assert(gamesCount > -1, "New game should have an id")
    }, consumerTag => {})
