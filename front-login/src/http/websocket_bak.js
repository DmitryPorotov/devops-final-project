...
class Websocket_bak {
    ...
    static async init(playerId) {
        if (!Websocket_bak.isInit) {
            Websocket_bak.isInit = true;
            Websocket_bak.eventHandlers = new Map();
            Websocket_bak.playerId = playerId;
            // window.mywebsoc = this;
        }
        else return;

        return Websocket_bak.makeSocket();
    }
    ...
    static makeSocket() {
        return new Promise((resolve => {
            ...
            const errorCb = (message) => {
                console.log(message);
                setTimeout(() => {
                    Websocket_bak.makeSocket().then(() => {
                        Websocket_bak.eventHandlers.forEach((cb) => {
                            Websocket_bak.socket.addEventListener('message', cb);
                        })
                    });
                }, 1000);
            };
    ...
    static onMessage(lobbyId, callBack) {
        ...
        Websocket_bak.socket.addEventListener('message', cbWrapper);
    }
    ...
    static offMessage(callBack) {
        ...
        Websocket_bak.socket.removeEventListener('message', wrapper);
    }
