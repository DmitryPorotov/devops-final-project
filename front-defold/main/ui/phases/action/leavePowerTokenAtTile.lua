...
local function on_leave_power_token_click()
	...
	event_dispatcher.trigger('ws_send', m)
end
...
function _M:init(houseType, tile_num)
	...
	event_dispatcher.on("leave_power_token_click", on_leave_power_token_click)
	leave_power_token:open()
end

function _M:clean_up()
	event_dispatcher.off('leave_power_token_click', on_leave_power_token_click)
